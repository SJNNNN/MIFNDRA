...
class MIFNDRA(nn.Module):
    def __init__(self, args):
        super(MIFNDRA, self).__init__()
        self.args = args
        self.lin_m = nn.Linear(args.ncRNA_number, args.in_feats, bias=False)
        self.lin_d = nn.Linear(args.drug_number, args.in_feats, bias=False)


        self.gcn_mm_1 = dglnn.SAGEConv(args.ncRNA_number, 128, 'mean')
        self.gcn_mm_2 = dglnn.SAGEConv(128, 64, 'mean')
        self.gcn_mm_3 = dglnn.SAGEConv(64, args.out_feats, 'mean')
        self.res_l_1 = nn.Linear(args.ncRNA_number, 64)

        self.gcn_dd_1 = dglnn.SAGEConv(args.drug_number, 128, 'mean')
        self.gcn_dd_2 = dglnn.SAGEConv(128, 64, 'mean')
        self.gcn_dd_3 = dglnn.SAGEConv(64, args.out_feats, 'mean')
        self.res_l_2 = nn.Linear(args.drug_number, 64)
    ...
    def forward(self, mm_graph, dd_graph, md_graph, ncRNA, drug, samples,triplet_samples):
        device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')
        mm_graph = mm_graph.to(device)
        dd_graph = dd_graph.to(device)
        md_graph = md_graph.to(device)
        ncRNA = ncRNA.to(device)
        drug = drug.to(device)

        res_mi = self.elu(self.res_l_1(ncRNA))
        res_di = self.elu(self.res_l_2(drug))

        md = torch.cat((self.lin_m(ncRNA), self.lin_d(drug)), dim=0)
        res_md = self.elu(self.res_l_3(md))

        res_mm = res_md[:self.args.ncRNA_number, :]
        res_dd = res_md[self.args.ncRNA_number:, :]
        res_mmdd = torch.cat((res_mm, res_dd), dim=0)

        emb_mm_sim_1 = self.elu(self.gcn_mm_1(mm_graph,ncRNA))
        ...
        emb_dd_sim_1 = self.elu(self.gcn_dd_1(dd_graph, drug))
        ...
        emb_ass_1 = self.elu(self.gcn_md_1(md_graph, torch.cat((self.lin_m(ncRNA), self.lin_d(drug)), dim=0)))
        ...
        emb_mm_ass = emb_ass_3[:self.args.ncRNA_number, :]
        emb_dd_ass = emb_ass_3[self.args.ncRNA_number:, :]
